...
from ICD.etl import dict_etl, Dict2, multi_hot
...
def stableness_eval(net, user, item, u2i, i2u, user_traits, item_traits, batch_size):
    ...
    new_user_traits = pred_net.get_user_profiles(dict_etl(user, u2i, batch_size=batch_size))
    new_item_traits = pred_net.get_item_profiles(dict_etl(item, i2u, batch_size=batch_size))
...
@eval_wrapper
def turning_point(net: (torch.nn.DataParallel, ICD), inc_train_df: pd.DataFrame,
                  dict2: Dict2, inc_dict2: Dict2,
                  i2k, kn, batch_size, ctx,
                  epsilon=1e-2, tolerance=1e-2, logger=logging, *args, **kwargs):
    # return True
    net = net.module if isinstance(net, torch.nn.DataParallel) else net
    valid_inc_users = list(set(inc_train_df["user_id"].tolist()) & set(dict2.u2i.keys()))
    valid_inc_items = list(set(inc_train_df["item_id"].tolist()) & set(dict2.i2u.keys()))
    ...
    user_traits: dict = net.get_user_profiles(dict_etl(set(users), dict2.u2i, batch_size=batch_size))
    item_traits: dict = net.get_item_profiles(dict_etl(set(items), dict2.i2u, batch_size=batch_size))
    ...
    u_dim = v_dim = None
    for uid, iid, ks, rs in tqdm(DataLoader(TensorDataset(
        ...
        item_grad.append(torch.norm(torch.cat([a.grad, b.grad], dim=-1), 2, dim=-1).to("cpu"))
    ...
    for user in tqdm(valid_inc_users, "TP Learner"):
        ...
        kl = entropy(delta_r_distribution, [sum(user_df["pred_r"]), len(user_df["pred_r"]) - sum(user_df["pred_r"])])
        h_delta_r = entropy(delta_r_distribution)

        if not ((delta_r_pos / n_i * r_neg / m_i) ** 2 + (delta_r_neg / n_i * r_pos / m_i) ** 2) or not beta:
            if entropy(delta_r_distribution, [r_pos, r_neg]) > 0:
                logger.info("################# user %s inf exceed ##################" % user)
        ...
        delta_user.append(
            delta * (kl + h_delta_r - loss_item_r - (m_i + n_i) / n_i * epsilon) / (
                    math.sqrt(
                        (delta_r_pos / n_i * r_neg / m_i) ** 2 + (delta_r_neg / n_i * r_pos / m_i) ** 2
                    ) * beta
            )
        )
    if delta_user and max(delta_user) > tolerance * (u_dim ** 0.5):
        logger.info("++++++++++++ %s > %s +++++++++++++" % (max(delta_user), tolerance * (u_dim ** 0.5)))
        return True
    ...
    for item in tqdm(valid_inc_items, "TP Item"):
        ...
        kl = entropy(delta_r_distribution, [sum(item_df["pred_r"]), len(item_df["pred_r"]) - sum(item_df["pred_r"])])
        h_delta_r = entropy(delta_r_distribution)

        if not (delta_r_pos / n_j * r_neg / m_j + delta_r_neg / n_j * r_pos / m_j) or not beta:
            if entropy(delta_r_distribution, [r_pos, r_neg]) > 0:
                logger.info("################# item %s inf exceed ##################" % item)
                logger.info(delta_r_pos, delta_r_neg, r_pos, r_neg)
                return True
            else:
                continue
        ...
        delta_item.append(
            delta * (kl + h_delta_r - loss_item_r - (m_j + n_j) / n_j * epsilon) / (
                    (delta_r_pos / n_j * r_neg / m_j + delta_r_neg / n_j * r_pos / m_j) * beta
            )
        )

    if delta_item and max(delta_item) > tolerance * (v_dim ** 0.5):
        logger.info("++++++++++++ %s > %s +++++++++++++" % (max(delta_item), tolerance * (v_dim ** 0.5)))
        return True
