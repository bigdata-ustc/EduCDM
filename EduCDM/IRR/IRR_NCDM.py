...
class NCDM(PointNCDM):
    def __init__(self, meta_data: dict, hidd_dim1=512, hidd_dim2=256, zeta=0.5):
        super(NCDM, self).__init__(meta_data, hidd_dim1, hidd_dim2)
        self.zeta = zeta

    def fit(self, train_data: pd.DataFrame, epoch: int, val_data: pd.DataFrame = None,  device="cpu", lr=0.002, batch_size = 64, silence=False) -> ...:
        ...
        optimizer = torch.optim.Adam(self.ncdm_net.parameters(), lr, weight_decay=1e-4)
        train_data = self.trainsform__(train_data, batch_size, shuffle=True)
        ...
        for e in range(epoch):
            ...
            if val_data is not None:
                eval_data = self.eval(val_data, device=device)
                print("[Epoch %d]\n%s" % (e, eval_data))
