...
def main(dataset="a0910",
    ...
    inc_train_df_list = list(
        inc_stream(train_data,
                   stream_size=int(len(train_data) // stream_num)))
    ICDNet = ICD(cdm, meta_data, epoch, weight_decay,
                 inner_metrics, logger, alpha, ctx)
    ICDNet.fit(inc_train_df_list, i2k, beta, warmup_ratio, tolerance,
                 max_u2i, max_i2u, hyper_tag, wfs)
